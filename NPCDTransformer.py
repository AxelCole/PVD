...
class QKVMultiheadAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        if self.use_flash_attn:
            out = flash_attn_func(q.half(), k.half(), v.half(), causal=False, dropout_p=self.dropout if self.training else 0)
...
class NPCDTransformer(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, t: torch.Tensor, feats: Optional[torch.Tensor] = None):
        if feats is None:
            x = coords
        else:
        ...
        pred_feats = pred[:, self.coords_dim:] if feats is not None else None

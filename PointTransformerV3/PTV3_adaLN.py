...
class PDNorm(PointModule):
    ...
    def forward(self, point):
        ...
        _, counts = torch.unique(point.batch, return_counts=True)
        if self.adaptive:
            assert "context" in point.keys()
            shift, scale = self.modulation(point.context).chunk(2, dim=1)
            shift, scale = shift.repeat_interleave(counts, dim=0), scale.repeat_interleave(counts, dim=0)
...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        self.final_layer = PointSequential(ln_layer(dec_channels[0]),
                                           nn.Linear(dec_channels[0], in_channels, bias=True))
        self.t_embedder = TimestepEmbedder(context_channels)
    ...
    def forward(self, data_dict):
        ...
        point['context'] = self.t_embedder(point.time) # shape (B, C)

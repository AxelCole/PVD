...
# Step 1: Duplicate points from smaller clusters to reach the target size
def get_balanced_indices(labels, unique_clusters, cluster_counts, target_size):
    """Generates indices to reach target size per cluster without duplicating actual data."""
    balanced_indices = []
    balanced_labels = []

    for cluster, size in zip(unique_clusters, cluster_counts):
        cluster_indices = np.where(labels == cluster)[0]
        n_samples_needed = target_size - size

        if n_samples_needed > 0:
            # Duplicate indices with replacement if needed
            additional_indices = np.random.choice(cluster_indices, size=n_samples_needed, replace=True)
            cluster_indices = np.hstack([cluster_indices, additional_indices])
        
        balanced_indices.extend(cluster_indices)
        balanced_labels.extend([cluster] * target_size)
...
class Uniform15KPC(Dataset):
    def __init__(self, root_dir, subdirs, tr_sample_size=10000,
                 ...
                 input_dim=3, use_mask=False, device=torch.device('cuda:0'),
                 output_dir=None, distance_threshold=8.0):
        ...
        np.savez(os.path.join(output_dir, 'train.npz'), self.all_points)
        
        # Step 1: Extract features for each point cloud
        clf = PointNetClassifier(devices=[device])
        features, preds = clf.features_and_preds(NpzStreamer(os.path.join(output_dir, 'train.npz')))

        # Step 2: Initial KMeans clustering
        kmeans_model = KMeans(n_clusters=24, random_state=42)
        kmeans_labels = kmeans_model.fit_predict(features)

        # Step 3: Refine clusters with Agglomerative Clustering
        print(distance_threshold)
        agglo_model = AgglomerativeClustering(n_clusters=None, distance_threshold=distance_threshold, linkage='ward')
        refined_labels = agglo_model.fit_predict(kmeans_model.cluster_centers_)[kmeans_labels]

        # Step 4: Balance clusters using indices only
        unique_clusters, cluster_counts = np.unique(refined_labels, return_counts=True)
        print(f"Cluster sizes before duplication: {(unique_clusters, cluster_counts)}")  # Display initial cluster sizes

        target_size = cluster_counts.max()  # Target size is the max cluster size

        balanced_indices, balanced_labels = get_balanced_indices(refined_labels, unique_clusters, cluster_counts, target_size)

        # Assign the balanced indices directly
        self.all_points = self.all_points[balanced_indices]
        self.cate_idx_lst = np.array(self.cate_idx_lst)[balanced_indices]
        self.all_cate_mids = np.array(self.all_cate_mids)[balanced_indices]
        
        # Final shuffle of the dataset after duplication
        self.shuffle_idx = list(range(len(self.all_points)))
        random.Random(38383).shuffle(self.shuffle_idx)
        self.all_points = self.all_points[self.shuffle_idx]
        self.cate_idx_lst = self.cate_idx_lst[self.shuffle_idx]
        self.all_cate_mids = self.all_cate_mids[self.shuffle_idx]

        # Check the final dataset size and cluster distribution
        print(f"New dataset size: {self.all_points.shape[0]}")
        print(f"Cluster sizes after duplication: {np.unique(balanced_labels[self.shuffle_idx], return_counts=True)}")
        np.save(os.path.join(output_dir, f'train_augmented_shuffled_{device}.npy'), self.all_points)
        np.save(os.path.join(output_dir, f'train_clusters_shuffled_{device}.npy'), balanced_labels[self.shuffle_idx])
...
class ShapeNet15kPointClouds(Uniform15KPC):
    def __init__(self, root_dir="data/ShapeNetCore.v2.PC15k",
        ...
        super(ShapeNet15kPointClouds, self).__init__(
            ...
            input_dim=3, use_mask=use_mask, device=device, output_dir=output_dir, distance_threshold=distance_threshold)

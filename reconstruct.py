...
class Model(nn.Module):
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        self.model = PointTransformerV3(no_conv=args.no_conv, qk_norm=args.qk_norm, cosine_attention=args.cosine_attention, wandb_run=args.wandb_run)
...
def subsample_point_cloud(point_cloud, num_points=2048):
    # Randomly sample `num_points` from `point_cloud`
    tr_out = point_cloud[:10000]  # Limit to first 10000 points if necessary
    tr_idxs = np.random.choice(tr_out.shape[0], num_points, replace=False)
    return tr_out[tr_idxs, :]
...
def main(opt):
    ...
    with torch.no_grad():
        tr_dataset, _ = get_dataset(opt.dataroot, opt.npoints, opt.category)
        m, s = torch.from_numpy(tr_dataset.all_points_mean), torch.from_numpy(tr_dataset.all_points_std)
        ...
        np_list = [
            np.load('/lustre/fswork/projects/rech/mdv/ucq62mm/PVD/data/ShapeNetCore.v2.PC15k/03001627/train/1a6f615e8b1b5ae4dbbc9440457e303e.npy'),
            np.load('/lustre/fswork/projects/rech/mdv/ucq62mm/PVD/data/ShapeNetCore.v2.PC15k/03001627/train/1a74a83fa6d24b3cacd67ce2c72c02e.npy'),
            np.load('/lustre/fswork/projects/rech/mdv/ucq62mm/PVD/data/ShapeNetCore.v2.PC15k/03001627/train/1aa07508b731af79814e2be0234da26c.npy'),
            np.load('/lustre/fswork/projects/rech/mdv/ucq62mm/PVD/data/ShapeNetCore.v2.PC15k/03001627/train/1ab4c6ef68073113cf004563556ddb36.npy'),
            np.load('/lustre/fswork/projects/rech/mdv/ucq62mm/PVD/data/ShapeNetCore.v2.PC15k/03001627/train/1ab42ccff0f8235d979516e720d607b8.npy')
        ]

        # Subsample each point cloud in np_list
        subsampled_list = [subsample_point_cloud(pc) for pc in np_list]

        # Stack subsampled point clouds into a single tensor
        x0 = torch.tensor(np.stack(subsampled_list), dtype=torch.float32)
        x0 = x0.permute(0, 2, 1)
        x0 = (x0 - m.permute(0,2,1))/s 
        print(x0.shape)

        if opt.cuda:
            x0 = x0.cuda()

        for t in range(500, 1001, 100):  # Include 50 in the range
            reco_path = os.path.join(outf_syn, f'reconstructed_from_ms{t}.pth')
            Path(reco_path).parent.mkdir(parents=True, exist_ok=True)
            x_recon = model.reconstruct(x0, t)
            torch.save(x_recon.cpu(), reco_path)  # Ensure x_recon is moved to CPU before saving if necessary
...
def parse_args():
    ...
    parser.add_argument('--workers', type=int, default=8, help='workers') # 8 CPUs per GPU

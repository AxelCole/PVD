...
class GaussianDiffusion:
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None):
        ...
        if self.loss_type == 'mse':
            ...
            assert eps_recon.shape == torch.Size([B, D])
...
class Model(nn.Module):
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        self.model = ResMLP(hidden_size=args.hidden_size, depth=args.depth, mlp_ratio=args.mlp_ratio)
...
class LatentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return {'idx':idx, 'features':self.data_tensor[idx]}
...
def train(gpu, opt, output_dir, noises_init):
    ...
    if should_diag:
        #run = wandb.init(project='LDM_MLP', id=opt.wandb_id, resume="allow", config=opt)
        run = wandb.init(project='LDM_MLP', config=opt)
    ...
    data_tensor = torch.tensor(features_train, dtype=torch.float32)
    train_dataset = LatentDataset(data_tensor)
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        if (epoch + 1) % opt.vizIter == 0:
            ...
            with torch.no_grad():
                ...
                x_gen_eval = model.gen_samples(new_x_chain(x, 256).shape, x.device, clip_denoised=False).detach().cpu().numpy()

                features = x_gen_eval
                features = torch.from_numpy(features).to(torch.device(f"cuda:{gpu}"))
                ...
                if should_diag:
                    ...
                    run.log({"fid_val": fid_val, "fid_train": fid_train})                    
                    logger.info(f'P-FID_val: {fid_val}, P-FID_train: {fid_train}')
...
def main():
    ...
    noises_init = torch.randn(features_train.shape)
...
def parse_args():
    ...
    parser.add_argument('--depth', type=int, default=2, help='number of ResMLP blocks')

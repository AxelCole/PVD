...
from NPCDTransformer import NPCDTransformer
...
class Model(nn.Module):
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        self.model = NPCDTransformer(coords_dim=3, feats_dim=0, width=args.width, layers=args.layers, heads=args.heads, dropout=args.attn_drop, use_flash_attn=args.enable_flash)
...
def train(gpu, opt, output_dir, noises_init):
    ...
    if should_diag:
        #run = wandb.init(project='npcd_vanilla_transformer', id=opt.wandb_id, resume="allow", config=opt)
        run = wandb.init(project='npcd_vanilla_transformer', config=opt)
...
def parse_args():
    ...
    parser.add_argument('--width', type=int, default=512, help='embedding dimension')
    parser.add_argument('--layers', type=int, default=12, help='number of attention blocks')
    ...
    parser.add_argument('--enable_flash', action='store_true', default=False, help='If set, flash attention will be applied')

...
def check_gradients(opt, logger, net):
    for name, param in net.named_parameters():
        if param.grad is not None and torch.isnan(param.grad).any():
            logger.info(f"NaNs detected in gradients of {name}")
            opt.vizIter = 1

def check_parameters(opt, logger, model):
    for name, param in model.named_parameters():
        if torch.isnan(param).any():
            logger.info(f"NaNs detected in parameters of {name}")
            opt.vizIter = 1
...
def train(gpu, opt, output_dir, noises_init):
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader):
            ...
            check_gradients(opt, logger, model)
            if opt.grad_clip is not None:
                torch.nn.utils.clip_grad_norm_(model.parameters(), opt.grad_clip)

            optimizer.step()
            check_parameters(opt, logger, model)
        ...
        if (epoch + 1) % opt.vizIter == 0 and should_diag:
            ...
            with torch.no_grad():
                ...
                x_gen_eval = model.gen_samples(new_x_chain(x, 25).shape, x.device, clip_denoised=True)
                x_gen_list = model.gen_sample_traj(new_x_chain(x, 1).shape, x.device, freq=40, clip_denoised=True)
...
def parse_args():
    ...
    parser.add_argument('--print_freq', default=1, help='unit: iter')

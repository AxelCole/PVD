...
from datasets.shapenet_data_pc_resampled import ShapeNet15kPointClouds
...
def get_dataset(dataroot, npoints, category, device=torch.device('cuda:0'), output_dir=None, distance_threshold=8.0):
...
def train(gpu, opt, output_dir, noises_init):
    ...
    ''' data '''
    train_dataset = get_dataset(opt.dataroot, opt.npoints, opt.category, torch.device(f"cuda:{gpu}"), tmp, opt.distance_threshold)
...
def main():
    ...
    ''' workaround '''
    train_dataset = get_dataset(opt.dataroot, opt.npoints, opt.category, torch.device('cuda:0'), output_dir, opt.distance_threshold)
...
def parse_args():
    ...
    parser.add_argument('--distance_threshold', type=float, default=8.0, help='Control how KMeans clusters are merged with Agglomerative Clsuetring')
